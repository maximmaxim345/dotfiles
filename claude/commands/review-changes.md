...
3. Check out the PR head. If `./` is already on it, review there. Otherwise run `gh pr checkout <pr>` in `./`, unless it has uncommitted changes or the current branch has unpushed commits. In that case create a worktree under `<repo>/.claude/worktrees/pr-<number>` and say why. Note the branch `./` was on before.
...
- In step 8, write to `./.claude/reviews/local-<branch>.md` and describe each finding with a suggested fix instead of a comment for someone else. Don't change any code until I ask.
- Skip steps 9 and 10.
